...
from .models import Usuario, Chat, Mensaje, PerfilWorker, PerfilClient, ProyectoWorker, Publicacion, Solicitud
...
def resenas_client(request, usuario_id):
    try:
        perfil = PerfilClient.objects.get(usuario__id=usuario_id)
    except PerfilClient.DoesNotExist:
        return Response({'error': 'Perfil no encontrado'}, status=404)

    # Por ahora devolvemos lista vacía hasta implementar el sistema de calificaciones
    return Response([])
# ====================
...
def publicaciones_worker(request):
    try:
        ...
        categorias = perfil.categorias  # lista desde JSONField
    except PerfilWorker.DoesNotExist:
        categorias = []

    # Mapeo: el perfil guarda "Plomero", la publicación guarda "plomero"
    ...
    # IDs de publicaciones a las que ya envió solicitud
    ya_enviadas = set(
    ...
    return Response({'publicaciones': data})
...
def elegir_worker(request, solicitud_id):
    ...
    if accion == 'aceptar':
        ...
        if chat_existente:
            # Mandar mensaje automático en el chat existente
...
def resenas_worker(request, usuario_id):
    try:
        perfil = PerfilWorker.objects.get(usuario__id=usuario_id)
    except PerfilWorker.DoesNotExist:
        return Response({'error': 'Perfil no encontrado'}, status=404)
